    ...
    return (
        <div className="flex flex-col" style={{ minHeight: "100dvh", background: "#1a120d" }}>
            <header className="flex items-center justify-between px-5 pt-12 pb-3">
                <button onClick={() => router.back()} className="h-10 w-10 rounded-full bg-white/10 flex items-center justify-center">
                    <span className="material-symbols-outlined text-white/70">arrow_back</span>
                </button>
                <h1 className="text-white font-semibold">Create Group</h1>
                <div className="w-10" />
            </header>

            <main className="flex-1 flex flex-col px-5 pb-6">
                <div className="mb-4">
                    <label className="text-white/50 text-xs uppercase tracking-wider mb-2 block">Group Name</label>
                    <input
                        type="text"
                        placeholder="Friday Night Eats 🍕"
                        value={name}
                        onChange={e => setName(e.target.value)}
                        className="w-full bg-white/10 rounded-xl px-4 py-3.5 text-white placeholder:text-white/40 focus:outline-none"
                    />
                </div>

                <div className="flex items-center gap-3 p-4 bg-white/5 rounded-xl mb-4">
                    <div className="h-11 w-11 rounded-lg bg-[#f46a25]/20 flex items-center justify-center">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-white/50 text-sm">Bengaluru</p>
                    </div>
                    <span className="material-symbols-outlined text-white/30">chevron_right</span>
                </div>

                <div className="flex-1">
                    <label className="text-white/50 text-xs uppercase tracking-wider mb-2 block">Who&apos;s Coming?</label>
                    <div className="flex items-center gap-3 mb-2">
                        <button onClick={invite} className="h-14 w-14 rounded-xl border-2 border-dashed border-white/20 flex items-center justify-center">
                            <span className="material-symbols-outlined text-white/40">add</span>
                        </button>
                        {members.map(m => (
                            <div key={m.id} className="relative">
                                <div className="h-14 w-14 rounded-xl bg-white/10 flex items-center justify-center text-2xl">{m.avatar}</div>
                    ...
                    <p className="text-white/40 text-sm">{members.length} ready to swipe</p>
                </div>
                ...
                <div className="flex gap-3 mb-4">
                    ...
                    <button onClick={invite} className="flex-1 bg-white/10 text-white py-3.5 rounded-xl flex items-center justify-center gap-2">
                        <span className="material-symbols-outlined">link</span>
                        Copy Link
                    </button>
                </div>

                <button onClick={() => router.push("/discover")} className="w-full py-4 bg-[#f46a25] text-white font-semibold text-lg rounded-full">
                    Start Swiping
                </button>
            </main>
        </div>
